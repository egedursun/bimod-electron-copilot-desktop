...
#
#  Project: Bimod.io™
#  File: voidforger_dashboard_views.py
#  Last Modified: 2024-11-25 00:32:27
#  Author: Ege Dogan Dursun (Co-Founder & Chief Executive Officer / CEO @ BMD™ Autonomous Holdings)
#  Created: 2024-11-25 00:32:27
#
...
from apps.chats.models import MultimodalVoidForgerChat, MultimodalVoidForgerChatMessage
from web_project import TemplateLayout


class DashboardView_VoidForger(TemplateView):
    def get_context_data(self, **kwargs):
        context = TemplateLayout.init(self, super().get_context_data(**kwargs))
        context['available_chats'] = MultimodalVoidForgerChat.objects.all()

        chat_id = self.request.GET.get('chat_id')

        if chat_id:
            selected_chat = get_object_or_404(MultimodalVoidForgerChat, id=chat_id)
            context['selected_chat'] = selected_chat

            context['chat_messages'] = MultimodalVoidForgerChatMessage.objects.filter(
                chat=selected_chat
            ).order_by(
                'sent_at'
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not chat_id or not message_text:
            ...
            return redirect('dashboards:voidforger_dashboard')
        ...
        try:
            _ = self.get_voidforger_response(
                chat=chat,
        ...
        return redirect(f"{request.build_absolute_uri('/dashboards/voidforger/')}?chat_id={chat_id}")

    @staticmethod
    def get_voidforger_response(chat, chat_history):
